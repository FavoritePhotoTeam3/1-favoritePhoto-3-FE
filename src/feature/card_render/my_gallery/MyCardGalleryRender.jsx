...
import style from "./MyCardGalleryRender.module.css";
import MyGalleryCard from "./components/card/myGalleryCard";

import { useSelector, useDispatch } from "react-redux";
import { setMyCards } from "./myGallerySlice";

import { useGetMyCardQuery } from "@hooks/query/useGetMyCardQuery";
...
const MyCardGalleryRender = () => {
  ...
  const { data, fetchNextPage, hasNextPage, isError, isFetching, status } =
    useGetMyCardQuery(limit, keyword, filterOptions);
  ...
  const renderCards = useMemo(() => {
    return myCards.map((data) => (
      <Link key={data.id} to={`/mygallery/${data.id}` }>
        <MyGalleryCard data={data} />
      </Link>
    ));
  }, [myCards]);
