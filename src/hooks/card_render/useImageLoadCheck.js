...
export const useImageLoadCheck = (onImagesLoaded) => {
  ...
  const checkImagesLoaded = () => {
    ...
    const handleLoad = (event) => {
      loadedImages++;
      console.log(`이미지 로드됨: ${loadedImages}/${picture.length}`);
    ...
    picture.forEach((picture) => {
      ...
      if (img.complete) {
        loadedImages++;
        console.log(`이미 로드된 이미지: ${loadedImages}/${picture.length}`);
      } else if (!listenersAdded.current) {
        console.log("이미지 로드 리스너 추가");
        img.addEventListener("load", handleLoad); // 리스너가 추가되지 않은 경우에만 리스너 추가
      }
    });

    if (loadedImages === picture.length) {
      console.log("모든 이미지가 이미 로드된 상태입니다.");
